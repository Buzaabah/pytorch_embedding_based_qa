...
class Evaluate():
    ...
    def load(self, name):
        return pickle.load(open('insurance_qa_python/'+name, 'rb'))
    ...
    def validate(self, validation=False):
        self.model.load_state_dict(torch.load("saved_model/answer_selection_model_cnnlstm", map_location=torch.device('cpu')))
